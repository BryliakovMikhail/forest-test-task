      ...
      el.forEach(function (subEl, subElIndex) {
        if (params.type === 'fraction') {
          subEl.querySelectorAll(classesToSelector(params.currentClass)).forEach(function (fractionEl) {
            fractionEl.textContent = '0' + params.formatFractionCurrent(current + 1);
          });
          subEl.querySelectorAll(classesToSelector(params.totalClass)).forEach(function (totalEl) {
            totalEl.textContent = '0' + params.formatFractionTotal(total);
          });
        }
...
swiperSlides.forEach(function (slide, index) {
  var heroNumber = slide.querySelector(".hero__number");
  var number = index + 1;
  heroNumber.textContent = 0 + number.toString();
});
var stopSwiperScroll = document.querySelectorAll('.stop-swiper-scroll');
stopSwiperScroll.forEach(function (el) {
